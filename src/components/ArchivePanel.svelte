                    ...
                    {#each postsByYear[year] as post (post.id)}
                        <div animate:flip={{ duration: 600 }} in:fade={{ duration: 150 }} out:fade={{ duration: 150 }} >
                            ...
                            {#if post.data.tags && post.data.tags.length > 0}
                                <div class="flex flex-wrap gap-1 pl-[calc(80px+1rem)] md:pl-[calc(120px+1rem)] pb-2">
                                    {#each post.data.tags as tag}
                                        <span class="px-2 py-0.5 text-xs rounded bg-[var(--button-hover-color)] text-[var(--text-color-70)] border border-[var(--button-border-color)]">
                                            {tag}
                                        </span>
                                    {/each}
                                </div>
